  ...
  return (
    <section className="page">
      <div className="pageTitle">
        <div><h2>Reports & Advanced Analytics</h2></div>
        <div className="actions">
          <button onClick={() => download('/reports/export?format=xlsx', 'takora-task-report.xlsx')}>Export Excel</button>
          <button onClick={() => download('/reports/export?format=pdf', 'takora-task-report.pdf')}>Export PDF</button>
          <button onClick={() => download('/reports/export?format=csv', 'takora-task-report.csv')}>Export CSV</button>
        </div>
      </div>
      ...
      <div className="grid2">
        <BarChart title="Department-wise Completed Tasks" data={data.departmentPerformance.map(d => ({ label: d.department, value: d.completed }))} />
